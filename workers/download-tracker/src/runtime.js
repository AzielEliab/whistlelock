...
function openapiSpec(origin) {
  return {
    ...
    info: {
      ...
      description: LIMITATION + " Suite mesh /v1/mesh/* PROXY to aziel-runtime (AZIEL_RUNTIME). Default OFF. QNM-BUILD-1.0 live|locked|isolated. QNS-CD-1.0 photon QNS1 packet transfer is a hub cite / Worker mesh cross-map only (no public qnsd proxy). No Node Gate. No auto-heal. Not anonymity. Aziel Eliab only.",
...
<p>ChatGPT: GPT Actions \u2192 Import from URL (no auth). Grok: import the OpenAPI as a custom tool, or MCP. Venice: HTTP tools. Claude, Cursor, Glama, and other MCP clients: POST the Worker or catalog MCP URL.</p>
...
<p>Suite mesh: <code>GET ${origin}/v1/mesh</code> PROXY to aziel-runtime. Default OFF. QNM-BUILD-1.0 live|locked|isolated. QNS-CD-1.0 photon QNS1 packet transfer (hub cite / Worker mesh cross-map only; no public qnsd proxy). No Node Gate. No auto-heal. Not anonymity. Author: Aziel Eliab only.</p>
...
export async function handleRuntimeApi(request, url) {
  ...
  if (path.startsWith("/v1/") || path === "/v1") {
    return json({ error: "not found", hint: "GET /v1/health  GET /v1/skill  POST /v1/hash-preview  POST /v1/canon-preview  GET /v1/mesh", limitation: LIMITATION }, 404);
  }
  return null;
}
